...
async function unifiedRespond(message, { session, events, neighborhood, nearbyHoods, conversationHistory, currentTime, validNeighborhoods, activeFilters, isSparse, isCitywide, isBorough, borough, matchCount, hardCount, softCount, excludeIds, suggestedNeighborhood, userHoodAlias, isLastBatch, exhaustionSuggestion, model } = {}) {
  ...
  // Structure: data at top (events, context), query at bottom (closest to generation)
  const userPrompt = `Current time (NYC): ${now}
...
Respond now.`;
  ...
  if (!parsed || !parsed.sms_text || typeof parsed.sms_text !== 'string') {
    console.error('unifiedRespond: invalid response:', raw);
    return {
      type: 'conversational',
...
// Tool schema for Anthropic tool_use in unifiedRespond
const UNIFIED_TOOL = {
  name: 'unified_response',
  description: 'Return your unified event recommendation response including SMS text',
  input_schema: {
    type: 'object',
    properties: {
      type: { type: 'string', enum: ['event_picks', 'conversational', 'ask_neighborhood'] },
      sms_text: { type: 'string', description: 'The SMS response text (max 480 characters)' },
      picks: { type: 'array', items: {
        type: 'object',
        properties: {
          rank: { type: 'integer' },
