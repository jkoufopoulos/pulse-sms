...
/**
 * Assembles the reasoning system prompt from REASON_SYSTEM base + conditional skill modules.
 * Used by the split-mode reasoning call (intent classification + event selection).
 */
function buildReasonPrompt(events, options = {}) {
  const { REASON_SYSTEM } = require('../prompts');
  const parts = [REASON_SYSTEM];
  ...
  // Tonight priority — skip for future-oriented queries
  if (events && events.length > 0) {
    const todayNyc = getNycDateString(0);
    const hasToday = events.some(e => {
      const d = e.date_local || e.day;
  ...
  // Conversation awareness (for understanding context, not formatting)
  if (options.hasConversationHistory) {
    parts.push(skills.conversationAwareness.text);
  }
...
/**
 * Assembles the rendering system prompt from RENDER_SYSTEM base + conditional skill modules.
 * Used by the split-mode rendering call (SMS text composition from validated picks).
 */
function buildRenderPrompt(events, options = {}) {
  const { RENDER_SYSTEM } = require('../prompts');
  const parts = [RENDER_SYSTEM];

  // Tonight priority — for day labels
  if (events && events.length > 0) {
    const todayNyc = getNycDateString(0);
    const hasToday = events.some(e => {
      const d = e.date_local || e.day;
  ...
  // Nearby suggestion — when picks are thin
  if (options.suggestedNeighborhood) {
    parts.push(`\nNEARBY SUGGESTION: Picks are thin — suggest ${options.suggestedNeighborhood} as an alternative (e.g. "Slim pickings tonight — ${options.suggestedNeighborhood} is nearby, want picks from there?").`);
  }

  // Single pick — when exactly 1 pick
  if (options.pickCount === 1) {
    parts.push(skills.singlePick.text);
  }
  ...
  // Conversation awareness — for temporal language in rendering
  if (options.hasConversationHistory) {
    parts.push(skills.conversationAwareness.text);
  }
...
module.exports = { buildUnifiedPrompt, buildReasonPrompt, buildRenderPrompt };
