...
function parseType(subject) {
  // Health alerts: "Pulse: 3 sources failing", "Pulse: 1 source failing"
  if (/source.?\s*failing/i.test(subject)) {
    return { type: 'health', alertType: undefined };
  }
  // Runtime alerts: "Pulse: slow_response", "Pulse: error_rate", etc.
  const match = subject.match(/^Pulse:\s*(.+)$/i);
...
async function main() {
  ...
  const emails = await fetchEmails('from:onboarding@resend.dev subject:Pulse newer_than:90d', 100);
